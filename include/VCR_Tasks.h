 ...
 * 1) Define the "init" function. Name this function your task name with _init_function() after it.
 *    This init's function's inputs MUST be the same for all init functions, taking in sysMicros
 *    and a taskInfo reference.
 * 2) Define the "run" function. Name this function your task name with _run_function() after it.
 *    Similar to the init function, this function's inputs MUST be sysMicros and taskInfo.
 ...
 */
bool test_task_init_function(const unsigned long& sysMicros, const HT_TASK::TaskInfo& taskInfo)
{
    hal_printf("Initialized function at %d (micros)\n", sysMicros);
    return true;
}

bool test_task_run_function(const unsigned long& sysMicros, const HT_TASK::TaskInfo& taskInfo)
{
    hal_printf("Ran function at %d (micros) for the %dth time\n", sysMicros, taskInfo.executions);
    return true;
}

HT_TASK::Task test_task = HT_TASK::Task(test_task_init_function, test_task_run_function, 1, 100000UL); // 100,000us is 10hz
 ...
 */
bool read_adc0_init_function(const unsigned long& sysMicros, const HT_TASK::TaskInfo& taskInfo)
{
...
bool read_adc0_run_function(const unsigned long& sysMicros, const HT_TASK::TaskInfo& taskInfo)
{
...
HT_TASK::Task read_adc0_task = HT_TASK::Task(test_task_init_function, test_task_run_function, 10, 1000UL); // 1000us is 1kHz
