...
void InverterInterface::send_INV_TEMP_DATA() 
{
    ...
    msg_out.inverter_temp_ro = HYTECH_inverter_temp_ro_toS(_feedback_data.temps.inverter_temp);
    msg_out.motor_temp_ro = HYTECH_motor_temp_ro_toS(_feedback_data.temps.motor_temp);
    msg_out.igbt_temp_ro = HYTECH_igbt_temp_ro_toS(_feedback_data.temps.igbt_temp);
...
void InverterInterface::send_INV_DYNAMICS_DATA() 
{
    INV1_DYNAMICS_t msg_out;

    msg_out.actual_power_w = _feedback_data.motor_mechanics.actual_power;
    msg_out.actual_torque_nm_ro = HYTECH_actual_torque_nm_ro_fromS(_feedback_data.motor_mechanics.actual_torque);
    msg_out.actual_speed_rpm = _feedback_data.motor_mechanics.actual_speed;

    CAN_util::enqueue_msg(&msg_out, &Pack_INV1_DYNAMICS_hytech, VCRCANInterfaceImpl::telem_can_tx_buffer, inverter_ids.inv_dynamics_id);
}

void InverterInterface::send_INV_TORQUE_LIMITS() //same as the send_INV_SETPOINT_COMMAND but going on telem_can instead
{
    INV1_CONTROL_INPUT_t msg_out;
