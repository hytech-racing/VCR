...
class DrivebrainInterface {
  public:
    ...
    void receive_drivebrain_torque_setpoint(const CAN_message_t &msg, unsigned long curr_millis);

    void handle_enqueue_suspension_CAN_data();

    void handle_send_ethernet_data(const hytech_msgs_VCRData_s &data);
    StampedDrivetrainTorqueCommand_s get_latest_data();
  ...
  private:
    ...
    StampedDrivetrainTorqueCommand_s _latest_drivebrain_command = {};
};
