...
DrivetrainCommand_s DrivebrainController::evaluate(const VCRData_s &state, unsigned long curr_millis)
{
    ...
    if( (curr_millis - last_torque_setpoint_timestamp) > _worst_message_latencies.worst_torque_setpoint_latency_so_far)
    {
        _worst_message_latencies.worst_torque_setpoint_latency_so_far = static_cast<int64_t>(curr_millis - last_torque_setpoint_timestamp);
    }

    bool timing_failure = (torque_setpoint_msg_too_latent || not_all_messages_recvd );
    ...
    {   
        auto db_cmd = db_input.get_command();
        // HACK im using the torque limits within the drivetrain command as the torque setpoints for now
        output.torque_limits = db_cmd.torque_setpoints;
        output.desired_speeds = {};
    }
    else
    {
