...
VCRControls::VCRControls(DrivetrainSystem *dt_system, uint32_t max_allowed_db_latency_ms) :
    _mode0_10_mps({1.0, 1.0, 5559.622, 20.0f, 10.0f}),
    _mode0_15_mps({1.0, 1.0, 8339.433, 20.0f, 10.0f}),
    _mode0_20_mps({1.0, 1.0, 11119.244, 20.0f, 10.0f}),
    ...
    _tc_mux({
        ...
        [this](const VCRData_s &state, unsigned long curr_millis) -> DrivetrainCommand_s { return _mode0_10_mps.evaluate(state, curr_millis); },
        [this](const VCRData_s &state, unsigned long curr_millis) -> DrivetrainCommand_s { return _mode0_15_mps.evaluate(state, curr_millis); },
        [this](const VCRData_s &state, unsigned long curr_millis) -> DrivetrainCommand_s { return _mode0_20_mps.evaluate(state, curr_millis); },
